...
fname = argv[1]
retfile = "Output\\Generated\\BYTECODE\\SBLC\\BYTECODE.sblc"
...
with open(fname, "r") as file:
    ...
    if(lines[0] == "Int main<Main> {" and file_read.endswith("\n}")):
        
        memstack.append("Function::Int::main")
...
# <RETURN_TYPE>: Void, CharacterArray, Character, Any, Int, IntU, Array, FloatingPointNumber32Bit, SystemLong64BitSigned, SystemLong64BitUnsigned
        ...
        for line in lines:
            ...
            if file_line.startswith("    Print *, "):
                ...
                if len(memstack) >= 32768:
                    exit()
            if file_line.startswith("    PrintI &, "):
                
                memstack.append("PrintVariable::Int::Next")
                ...
                if len(memstack) >= 32768:
                    exit()
            if file_line.startswith("    Int "):
                
                kccc = file_line.split(" :: ")
                kccc.pop(0)
                a = f"DeclareVariable::Int::Next"
                b = kccc[0]
                memstack.append(a)
                memstack.append(b)
                ...
                if len(memstack) >= 32768:
                    exit()
            elif file_line.startswith("Int main<Main> {"):
                pass
